                ...
                columns: [
                    {
                        stack: [
                            ...
                            { text: `S/. ${Number(stats.revenue_month || 0).toLocaleString('es-PE', { minimumFractionDigits: 2 })}`, style: 'statValue' }
                        ],
                        alignment: 'center'
                    },
                    {
                                ...
                                table: {
                                    ...
                                    body: [
                                        ...
                                        ...stats.stockAlerts.map((s: any) => [
                                            ...
                                            { text: String(s?.amount || 0), style: 'tableCell', alignment: 'center', color: '#EF4444', bold: true }
                                        ])
                                    ]
                                },
                                layout: 'lightHorizontalLines'
                            } : { text: 'Stock óptimo en todos los productos.', italics: true, fontSize: 10, color: '#059669' }
                        ]
                    }
                ],
                margin: [0, 0, 0, 20]
                    ...
                    body: [
                        ...
                        ...stats.revenueData.map((r: any) => {
                            ...
                            return [
                                ...
                                { text: `S/. ${income.toLocaleString('es-PE', { minimumFractionDigits: 2 })}`, style: 'tableCell', alignment: 'right' },
                                { text: `S/. ${cost.toLocaleString('es-PE', { minimumFractionDigits: 2 })}`, style: 'tableCell', alignment: 'right' },
                                { 
                ...
                table: {
                    ...
                    body: [
                        ...
                        ...stats.recentActivity.map((a: any) => [
                            { text: a.date ? new Date(a.date).toLocaleDateString('es-ES') : 'N/A', style: 'tableCell' },
                            { text: String(a.treatment_name || 'N/A'), style: 'tableCell' },
                            { text: String(a.nurse_name || 'N/A'), style: 'tableCell' },
                            { text: `S/. ${Number(a.total_session || 0).toLocaleString('es-PE', { minimumFractionDigits: 2 })}`, style: 'tableCell', alignment: 'right' }
                        ])
                    ]
                },
                layout: 'lightHorizontalLines'
        ...
        try {
            ...
            Promise.resolve(pdfDocPromise).then((pdfDoc) => {
                ...
                pdfDoc.on('error', (err: any) => {
                    console.error('❌ Error en stream de PDF de Gestión:', err);
                    reject(err);
                });
                pdfDoc.end();
            }).catch(reject);
